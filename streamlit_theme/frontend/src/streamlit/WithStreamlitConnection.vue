<template>
  <div>
    <!--  Error boundary. If the wrapped component threw an error, display it. -->
    ...
    <!-- 
      ...
      Don't render until we've gotten the first RENDER_EVENT from Streamlit.
...
export default defineComponent({
  ...
  setup() {
    ...
    // Set up event listeners, and signal to Streamlit that it is ready. It
    // won't render the component until it receives the first RENDER_EVENT.
    onMounted(() => {
      Streamlit.events.addEventListener(Streamlit.RENDER_EVENT, onRenderEvent)
      Streamlit.setComponentReady()
    })
    onUpdated(() => {
      // If the slot threw an error, display it in render(). In this case, the
      // slot won't be mounted and therefore won't call `setFrameHeight` on its
      // own. It is done here so that the rendered error will be visible.
